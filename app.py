...
STYLE_CONFIG = {
    ...
    "社會案件 (Justice Alert)": {"theme": "Crime Scene Noir, Legal Justice", "ui": "CCTV grain, Map data overlays, Forensic textures", "palette": "Concrete Grey, Caution Yellow, Police Blue", "highlight": "Safety Orange"},
    "體育競技 (Victory Orange)": {"theme": "Sports High-Energy", "ui": "Carbon fiber, Kinetic speed lines, Stadium spotlights", "palette": "Electric Orange, Graphite Grey", "highlight": "Vivid Neon Yellow"},
    "全球財經 (Elite Obsidian)": {"theme": "High-end Financial Dashboard", "ui": "Aluminum frames, Holographic streams, Glass refractions", "palette": "Deep Navy, Gold, Cyan", "highlight": "Electric Cyan"},
    "突發重磅 (Breaking Alert)": {"theme": "Emergency Alert, Glossy Crimson", "ui": "Radial Motion Blur, Red internal glow", "palette": "Signal Red, White, Black", "highlight": "Bright Vivid Yellow"},
    "選情政論 (Democracy Grey)": {"theme": "Political Election Studio", "ui": "Matte Metallic, Star patterns, Marble textures", "palette": "Slate Grey, Navy Blue", "highlight": "Vibrant Scarlet Red"},
    "科技政策 (Cyber Policy)": {"theme": "Digital Policy Hub", "ui": "Poly-grid overlays, Ray-traced refraction", "palette": "Steel Blue, Neon Cyan", "highlight": "Neon Lime Green"},
    "綠能永續 (Eco-Future)": {"theme": "Sustainability & ESG", "ui": "Natural leaf textures, Outdoor bokeh", "palette": "Emerald Green, Leaf Green", "highlight": "Sunlight Gold"},
    "現代民俗 (Modern Festive)": {"theme": "Modern Folk, Rich Vermilion", "ui": "Silk texture, Traditional cloud patterns", "palette": "Vermilion Red, Gold", "highlight": "Imperial Gold"},
    "生醫科技 (Clinical White)": {"theme": "Medical & Bio-Tech", "ui": "Sanitized clinical surfaces, DNA helix motifs", "palette": "Pristine White, Sky Blue", "highlight": "Bright Sky Blue"}
}
...
def build_final_prompt(title, left_in, right_in, style_name, header_mode, icon_style, ai_autonomy):
    ...
    # 標題大大大權重：MEGA SIZE 300%
    HEADER_WEIGHT = "TITLE FONT SIZE: MEGA LARGE (300% of normal text). ABSOLUTE VISUAL HIERARCHY."
    if "兩行" in header_mode:
        HEADER_WEIGHT += " USE TWO-LINE STACKED LAYOUT. VERTICAL IMPACT."
    else:
        HEADER_WEIGHT += " USE SLEEK ONE-LINE HORIZONTAL LAYOUT."

    # 🛑 符號矩陣與空間避讓後台鎖定
    SYMBOL_LOGIC = f"""
[STRICT SYMBOL & SPACE TRANSFORMATION]
- DOUBLE QUOTES (" "): Highlight text within using {style['highlight']}. REMOVE quotes in final image.
- LENTICULAR BRACKETS (【 】): Render text as Sub-headers on a color block. FONT SIZE: LARGE. REMOVE brackets.
- (PARENTHESES): KEEP text and brackets as is. NO color change.
- [圖----slug圖]: ASSET PROTECTION ZONE. Leave this area 100% BLANK and EMPTY. NO text, effects, or backgrounds should overlap this coordinate. DELETE the label text.
- [效果說明]: (e.g., [對話框], [圓餅圖], [日曆效果], [筆刷效果], [蓋章效果], [icon]). Render visual only. COMPLETELY DELETE instruction text.
...
"""
    ...
    return f"""
[SYSTEM V11.1: ASSET PROTECTION] CANVAS: 1920x1080. {TICKER_VOID}
{HEADER_WEIGHT}
{SYMBOL_LOGIC}
[IMPORTANT] If "[圖----slug圖]" is present, leave a VOID area. DO NOT overlap it with any DATA_A or DATA_B text or effects.
...
<body>
    ...
    <script>
        ...
        document.getElementById('fgInput').onchange = e => { Array.from(e.target.files).forEach(f => { const r = new FileReader(); r.onload = ev => { const n = new Image(); n.onload = () => { fgImages.push({ img: n, x: 500, y: 300, w: 400, h: 400/(n.width/n.height), aspectRatio: n.width/n.height }); render(); }; n.src = ev.target.result; }; r.readAsDataURL(f); }); };
...
# ==========================================
# 4. Streamlit 介面 (全方位旗艦佈局)
# ==========================================
st.set_page_config(page_title="Visual Director v11.1", layout="wide")
st.title("🎬 Visual Director v11.1 - 製作人旗艦整合版")
st.caption("Producer Huifen Edition | 標題霸權與 slug 避讓鎖定")
...
with tab1:
    ...
    with st.expander("❓ 如何寫出專業鏡面？ (符號與效果說明)", expanded=False):
        st.markdown("### 🔠 符號語法與後台轉換矩陣")
        st.table([
            {"語法": '"文字"', "視覺動作": "文字高亮變色", "成品處理": "❌ 移除引號"},
            {"語法": "【文字】", "視覺動作": "色塊小標 (字體較大)", "成品處理": "❌ 移除括號"},
            {"語法": "[圖----slug圖]", "視覺動作": "🛡️ 劃定素材留白特區", "成品處理": "❌ 移除文字"},
            {"語法": "[效果說明]", "視覺動作": "轉化為圖示或 UI 物件", "成品處理": "❌ 移除說明文字"},
            {"語法": "[換行]", "視覺動作": "標題強制換行", "成品處理": "❌ 移除標籤"}
        ])
        st.caption("效果說明範例：[對話框][圓餅圖][日曆效果---(日期)][筆刷效果][蓋章效果][icon]")

    st.divider()
    col_l, col_r = st.columns([1.2, 0.8])
    
    with col_l:
        st.subheader("📋 鏡面內容編輯 (預設：萬華喪屍案)")
        header_mode = st.radio("標題強度：", ["兩行大標題 (超級強調)", "單行大標題 (專業政論)"], horizontal=True)
        
        # --- 萬華專案內容歸位 ---
        default_title = '"芬太尼喪屍"入侵萬華?[換行]\n"對折人"驚悚影像曝光' if "兩行" in header_mode else '"芬太尼喪屍"入侵萬華? "對折人"驚悚影像曝光'
        title_in = st.text_area("鏡面主標題 (AI 將字體設為 300% 大大大)", value=default_title, height=100)
        
        c1, c2 = st.columns(2)
        with c1:
            left_in = st.text_area(
                "區塊 A 內文", 
                value='[threads icon] \n[網友icon] 網友:[對話框] \n我以為我在舊金山\n[圖----網友圖佔版面1/2]\n北市萬華1男子\n遭疑吸"芬太尼"',
                height=250
            )
        with c2:
            right_in = st.text_area(
                "區塊 B 內文", 
                value='【"芬太尼"小檔案】\n●第二級毒品\n●人造鴉片類藥物\n  約嗎啡50-100倍\n●美加地區濫用\n  造成死亡案例\n\n刑事局:\n112年起僅台南查獲1件\n觀看影像確有吸毒樣子',
                height=250
            )
    ...
    with col_r:
        ...
        st.info(f"💡 當前高亮色：{STYLE_CONFIG[s_style]['highlight']}")

    if title_in:
        st.divider()
        st.subheader("🔥 最終生成：Gemini 繪圖指令")
        # 參數對位完全修正：(title, left, right, style, header, icon, ai)
        ...
        st.success("☝️ 素材避讓與標題霸權協定已啟動。AI 將在 slug 圖區域留白，並強制主標大大大。")

with tab2:
    st.subheader("🛠️ 華視打洞機 v66 (旗艦無損鑲嵌區)")
    st.caption("開啟 AI 底圖 $\rightarrow$ 插入前景（Slug 圖）$\rightarrow$ 粉紅筆開窗貼合 $\rightarrow$ 導出 1080p")
    # 鑲嵌製作人完整 v66 代碼
