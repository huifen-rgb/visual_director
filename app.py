...
# ==========================================
st.set_page_config(page_title="Visual Director v15.2", layout="wide")

# ==========================================
# 1. 內部門禁系統 (Secrets: APP_PASSWORD)
# ==========================================
_CORRECT_PASSWORD = os.environ.get("APP_PASSWORD", "CTS2026GO")
...
def check_password():
    ...
    st.title("🎬 Visual Director 內部系統 v15.2")
...
# ==========================================
STYLE_CONFIG = {
    "民生消費 (Fluid Analytics)": {"theme": "Consumer TRENDS, Fluid & Advanced", "ui": "Organic shapes, Frosted glass panels, Color overlays, Soft shadows", "palette": "Soft Beige, Lifestyle Blue", "highlight": "Vibrant Sunburst Orange"},
    "社會案件 (Justice Alert)": {"theme": "Crime Scene Noir, Legal Justice", "ui": "CCTV grain, Map data overlays, Forensic lighting textures, Caution tape", "palette": "Concrete Grey, Police Blue", "highlight": "Safety Orange"},
    "體育競技 (Victory Orange)": {"theme": "High-Energy Sports Broadcast", "ui": "Carbon fiber textures, Kinetic speed lines, Stadium spotlights, Metal mesh", "palette": "Graphite Grey, Stark White", "highlight": "Electric Orange"},
    "全球財經 (Elite Obsidian)": {"theme": "High-end Financial Dashboard", "ui": "Anodized Aluminum frames, Holographic streams, Glass refractions, Grid lines", "palette": "Deep Navy, Gold", "highlight": "Electric Cyan"},
    "突發重磅 (Breaking Alert)": {"theme": "Emergency Alert, High-Gloss Crimson", "ui": "Radial Motion Blur, Glassy UI panels, Red internal glow, Danger pulse", "palette": "Signal Red, Black", "highlight": "Bright Vivid Yellow"},
    "選情政論 (Democracy Grey)": {"theme": "Political Election Studio", "ui": "Matte Metallic, Star patterns, Studio spotlighting, Marble textures, Columns", "palette": "Slate Grey, Navy Blue", "highlight": "Vibrant Scarlet Red"},
    "科技政策 (Cyber Policy)": {"theme": "Digital Policy Hub", "ui": "Poly-grid overlays, Ray-traced refraction, Semi-transparent glass, Data nodes", "palette": "Steel Blue, Silver", "highlight": "Neon Cyan"},
    "綠能永續 (Eco-Future)": {"theme": "Sustainability & ESG Focus", "ui": "Natural leaf textures, Soft outdoor bokeh, Organic glass, Recycled paper", "palette": "Emerald Green, Leaf Green", "highlight": "Sunlight Gold"},
    "現代民俗 (Modern Festive)": {"theme": "Modern Folk, Rich Vermilion Aesthetic", "ui": "Lacquered Wood finish, Silk texture, Traditional cloud patterns, Gold leaf", "palette": "Vermilion Red, Deep Charcoal", "highlight": "Imperial Gold"},
    "生醫科技 (Clinical White)": {"theme": "Medical & Bio-Tech Innovation", "ui": "Sanitized clinical surfaces, DNA helix motifs, Hexagonal grid, Lab glass", "palette": "Pristine White, Navy Blue", "highlight": "Bright Sky Blue"}
}

# ==========================================
# 3. 核心指令引擎 v15.2 (右下禁區優化與引號銷毀鎖定)
# ==========================================
def build_final_prompt(title, left_in, right_in, style_name, header_mode, s_layout, icon_style, ai_autonomy, ticker_lock):
    style = STYLE_CONFIG[style_name]
    
    # 🛑 跑馬燈禁區優化 (從物理真空改為純背景覆蓋，內容緊湊排版)
    if ticker_lock:
        TICKER_VOID = (
            f"[HARD LIMIT] Zone ($1332 < X < 1920$, $990 < Y < 1080$) is for Hardware Ticker. "
            f"NO text, NO icons, NO UI panels are allowed in this exact pixel range. "
            f"ONLY continuous style background texture. Pack all content tightly into the remaining space."
        )
    else:
        TICKER_VOID = "[FULL CANVAS ACCESS]"
    ...
    # 🟢 局部質感切換
    if icon_style == "3D":
        icon_logic = "ICON TEXTURE: 3D Volumetric solids, physically based rendering (PBR), realistic tactical depth."
    else:
        icon_logic = "ICON TEXTURE: Flat 2D Vector silhouettes, minimalist design, solid colors."

    # 🟢 AI 色彩主權
    if ai_autonomy:
        color_logic = f"COLOR_STRATEGY: DYNAMIC_CONTEXTUAL. Analyze sentiment of '{title}' and create a professional news palette. Style: {style_name}."
        hi_color_ref = "the dynamic highlight color derived from content"
    else:
        color_logic = f"COLOR_STRATEGY: FIXED. Palette: {style['palette']}."
        hi_color_ref = style['highlight']

    SYMBOL_PROTOCOL = f"""
[STRICT SYMBOL MATRIX PROTOCOL]
- "雙引號": Highlight text with {hi_color_ref}. **CRITICAL**: DELETED quotes after coloring. NO quotes (" ") allowed in final pixels.
- \u3010方頭括號\u3011: Render text as Sub-headers on a color block. REMOVE brackets.
- [圖----slug圖]: ASSET PROTECTION ZONE. 100% CLEAN VOID. 20px buffer. 
  **ABSOLUTE EDGE CLEANLINESS**: NO overlays, shadows, or text may touch these zones. DELETE the slug label.
- [效果說明] & [icon]: {icon_logic} | Render visual only, DELETE label text.
- [筆刷效果]: Render as ARTISTIC PAINTED STROKE BACKGROUND. NO physical tool icons.
- [蓋章效果]: Render as DISTRESSED INK STAMP MARK OVERLAY. NO physical stamp icons.
- [換行]: Force manual line break.
"""
    visual_detail = f"STYLE: {style_name} | UI TEXTURE: Professional Flat Broadcast Graphics ({style['ui']}) | LAYOUT: {s_layout}"
    
    return f"[SYSTEM V15.2] CANVAS: 1920x1080. {TICKER_VOID}\n{HEADER_W}\n{SYMBOL_PROTOCOL}\n{visual_detail} | {color_logic} | CONTENT: TITLE={title} | DATA_A={left_in} | DATA_B={right_in} Traditional Chinese ONLY."

# ==========================================
# 4. 華視打洞機 v66 (100% 原始碼完全嵌入)
# ==========================================
HOLE_PUNCHER_V66 = """
<!DOCTYPE html>
<html lang="zh-TW">
...
<body>
    ...
    <script>
        ...
        selfieSegmentation.onResults(res => {
            ...
            aiCtx.save(); aiCtx.filter = 'blur(2.5px)';
        ...
        function render() {
            ...
            const drawStack = (targetCtx) => {
                targetCtx.save(); if(document.getElementById('autoLayoutCheck').checked) { targetCtx.filter = 'blur(40px) brightness(0.6)'; targetCtx.drawImage(img, -50, -50, W + 100, H + 100); targetCtx.filter = 'none'; }
        ...
        function drawUI(s) {
            wCtx.save(); let sel = activeItem.type === 'bg' ? bgImages[activeItem.index] : (activeItem.type === 'fg' ? fgImages[activeItem.index] : null);
            if(sel && !isDrawing) {
                wCtx.setLineDash([5, 5]); wCtx.strokeStyle = activeItem.type === 'bg' ? '#ffeb3b' : '#00e5ff';
                wCtx.lineWidth = 2; wCtx.strokeRect(sel.x, sel.y, sel.w, sel.h);
                const hX = sel.x + sel.w; const hY = sel.y + sel.h;
                wCtx.beginPath(); wCtx.arc(hX, hY, HANDLE_SIZE/2, 0, Math.PI*2);
                wCtx.fillStyle = wCtx.strokeStyle; wCtx.fill(); wCtx.strokeStyle = "#fff"; wCtx.lineWidth = 3; wCtx.stroke();
            }
        ...
        function exportManual() { const manualText = `華視打洞機 v66 - 操控優化版\\n\\n【操控更新】\\n1. 縮放優化：縮放手把加大至 24px。\\n2. 智能指標：滑鼠移到角落會顯示 ↘ 縮放圖示。\\n3. 即時透底：左側作業區即時預覽合成效果。`; const blob = new Blob([manualText], { type: 'text/plain' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = '使用說明.txt'; a.click(); }
...
# ==========================================
# 5. Streamlit 主 UI (v15.2 定案旗艦版)
# ==========================================
st.title("🎬 Visual Director v15.2 - 製作人最終定案旗艦版")
st.caption("Producer Huifen Edition | 右下禁區自然融合、引號/標籤強制滅跡、打洞機全員歸位")

tab1, tab2 = st.tabs(["🚀 第一步：鏡面指令", "🖍️ 第二步：華視打洞機"])

with tab1:
    with st.expander("❓ 鏡面符號矩陣與轉換原則 (同事導航手冊)", expanded=True):
        st.markdown("### 🔠 鏡面符號矩陣")
        st.table([
            {"符號語法": '"文字"', "視覺動作": "重點文字變色 (由 AI 依主題選色)", "後製處理": "❌ 移除雙引號 (不留任何像素)"},
            {"符號語法": "【文字】", "視覺動作": "色塊小標 (字大大大)", "後製處理": "❌ 移除方頭括號"},
            {"符號語法": "[圖----slug圖]", "視覺動作": "🛡️ 劃定素材留白特區", "後製處理": "❌ 移除標籤與內含 Slug 文字"},
            {"符號語法": "[效果說明]", "視覺動作": "轉化為視覺圖示 (如：[對話框])", "後製處理": "❌ 移除標籤文字"},
            {"符號語法": "[筆刷/蓋章]", "視覺動作": "轉化為風格化底紋/標記", "後製處理": "❌ 禁止實體工具出現"},
            {"符號語法": "[換行]", "視覺動作": "強制標題斷行指令", "後製處理": "❌ 移除標籤"}
        ])
    ...
    with col_l:
        ...
        # 使用圖上的 皮克敏 地球日 內容
        title_in = st.text_area("鏡面主標題", value='"皮克敏地球日"拚種花', height=100)
        c1, c2 = st.columns(2)
        with c1: 
            left_in = st.text_area("區塊 A 內文", value='【世界地球日】\n[日曆icon----4/22圖]\n4/22 12:00 - 4/27 16:00\n專屬"Party Walk"種花活動\n\n[圖----皮克敏世界圖]', height=250)
        with c2: 
            right_in = st.text_area("區塊 B 內文", value='【推動永續環保】\n[電腦icon]\n數萬名玩家種"虛擬花朵"\n現實世界\n步行取代短途車程\n[ icon----\u26fd]', height=250)
    
    with col_r:
        st.subheader("🛠️ 規格與美學庫")
        ticker_lock = st.toggle("🔒 鎖定跑馬燈區域 (限縮排斥力)", value=True)
        s_style = st.selectbox("旗艦風格庫", list(STYLE_CONFIG.keys()), index=1)
        ai_sovereignty = st.toggle("✨ 啟動 AI 視覺主權", value=True, help="AI 會分析標題，動態生成最精準的新聞配色。")
        
        r1, r2 = st.columns(2)
        with r1: s_layout = st.radio("排版模式", ["GRID", "DYNAMIC"], horizontal=True)
        with r2: s_icon = st.radio("ICON 質感", ["2D", "3D"], index=1, horizontal=True)
        
        st.info(f"🎨 Style ID: {s_style}")

    if title_in:
        st.divider()
        st.subheader("🔥 最終生成：Gemini 繪圖指令")
        final_cmd = build_final_prompt(title_in, left_in, right_in, s_style, h_mode, s_layout, s_icon, ai_sovereignty, ticker_lock)
