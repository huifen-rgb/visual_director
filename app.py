...
STYLE_CONFIG = {
    ...
    "綠能永續 (Eco-Future)": {"theme": "Sustainability & ESG", "ui": "Natural leaf textures, Soft outdoor bokeh, Organic glass, Recycled paper", "palette": "Emerald Green, Leaf Green, Soft White", "highlight": "Sunlight Gold"},
    "現代民俗 (Modern Festive)": {"theme": "Modern Folk, Rich Vermilion", "ui": "Lacquered Wood finish, Silk texture, Traditional cloud patterns, Gold leaf", "palette": "Vermilion Red, Gold, Deep Charcoal", "highlight": "Imperial Gold"},
    "生醫科技 (Clinical White)": {"theme": "Medical & Bio-Tech", "ui": "Sanitized clinical surfaces, DNA helix motifs, Hexagonal grid, Laboratory glass", "palette": "Pristine White, Sky Blue, Navy", "highlight": "Bright Sky Blue"}
}
...
def build_final_prompt(title, left_in, right_in, style_name, header_mode, icon_style, ai_autonomy, ticker_lock):
    ...
    # 🛑 1. 588x90 跑馬燈絕對禁區
    TICKER_VOID = "[ABSOLUTE PHYSICAL VOID] Bottom-Right ($1332 < X < 1920$, $990 < Y < 1080$). NO text, NO icons. Background blends naturally but remains content-free." if ticker_lock else "[FULL CANVAS ACCESS]"

    # 🛑 2. 標題霸權 (300% 大大大！)
    HEADER_W = "TITLE FONT SIZE: MEGA LARGE (300% size). ABSOLUTE VISUAL DOMINANCE OVER DATA."
    if "兩行" in header_mode: HEADER_W += " USE TWO-LINE STACKED LAYOUT (MAX IMPACT)."

    # 🛑 3. 鏡面符號矩陣 (Symbol Matrix Protocol)
    SYMBOL_PROTOCOL = f"""
[STRICT SYMBOL TRANSFORMATION]
- "雙引號": Highlight text using {style['highlight']}. REMOVE quotes in final image.
...
- [效果說明]: (e.g. [對話框], [圓餅圖], [日曆效果], [蓋章], [icon]). Render visual ONLY, DELETE text.
...
"""
    ...
    return f"""
[SYSTEM V11.9] CANVAS: 1920x1080. {TICKER_VOID}
{HEADER_W}
{SYMBOL_PROTOCOL}
[IMPORTANT] If "[圖----slug圖]" is present, leave area VOID. DO NOT overlap with any text or UI.
...
<body>
    ...
    <script>
        ...
        // 🛑 修復：動作開始前執行 saveHistory，確保 Undo 絕對有效
        workCanvas.onmousedown = e => {
            ...
            activeItem = { type: null, index: -1 }; commitShape(); isDrawing = true; startX = p.x; startY = p.y;
        ...
        document.getElementById('downloadBtn').onclick = () => { commitShape(); const a = document.createElement('a'); a.download = 'Final_CG.png'; a.href = previewCanvas.toDataURL('image/png'); a.click(); };
        window.onkeydown = e => { if(e.key.toLowerCase()==='z'&&e.ctrlKey) { e.preventDefault(); undo(); } };
    </script>
</body>
</html>
"""
...
# 4. Streamlit 介面佈局
# ==========================================
st.set_page_config(page_title="Visual Director v11.9", layout="wide")
st.title("🎬 Visual Director v11.9 - 旗艦功能總歸位版")
st.caption("Producer Huifen Edition | 符號矩陣、美學細節、打洞機手感 100% 同步")
...
with tab1:
    with st.expander("❓ 如何寫出專業鏡面？ (符號矩陣與空間避讓)", expanded=True):
        ...
        st.table([
            ...
            {"語法": "(圓括號)", "視覺動作": "文字不變色、保持輔助資訊感", "成品處理": "✅ 保留圓括號"},
    ...
    with col_r:
        ...
        s_style = st.selectbox("旗艦風格庫 (10 大全規格)", list(STYLE_CONFIG.keys()), index=3) 
...
with tab2:
    st.subheader("🛠️ 華視打洞機 v66 (完全歸位版)")
    st.caption("開啟底圖(縮放OK) $\rightarrow$ 插入前景 $\rightarrow$ 粉紅筆(Undo OK) $\rightarrow$ 導出 PNG")
