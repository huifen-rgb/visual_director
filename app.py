...
# ==========================================
# 1. 旗艦風格庫 (10 大全規格：細節 100% 還原)
# ==========================================
STYLE_CONFIG = {
    "民生消費 (Fluid Analytics)": {"theme": "Consumer TRENDS, Fluid", "ui": "Organic shapes, Frosted glass, Color overlays", "palette": "Soft Beige, Blue, Red", "highlight": "Vibrant Sunburst Orange"},
    "社會案件 (Justice Alert)": {"theme": "Crime Scene Noir", "ui": "CCTV grain, Map data, Forensic lighting", "palette": "Grey, Yellow, Police Blue", "highlight": "Safety Orange"},
    "體育競技 (Victory Orange)": {"theme": "Sports High-Energy", "ui": "Carbon fiber, Kinetic lines, Stadium spotlights", "palette": "Orange, Graphite, White", "highlight": "Vivid Neon Yellow"},
    "全球財經 (Elite Obsidian)": {"theme": "High-end Financial Dashboard", "ui": "Aluminum, Holographic streams, Glass refractions", "palette": "Navy, Gold, Cyan Data", "highlight": "Electric Cyan"},
    "突發重磅 (Breaking Alert)": {"theme": "Emergency Alert, High-Gloss Crimson", "ui": "Radial Blur, UI panels, Red glow", "palette": "Red, White, Black", "highlight": "Bright Vivid Yellow"},
    "選情政論 (Democracy Grey)": {"theme": "Political Studio", "ui": "Matte Metallic, Star patterns, Marble, Columns", "palette": "Slate Grey, Navy Blue", "highlight": "Vibrant Scarlet Red"},
    "科技政策 (Cyber Policy)": {"theme": "Digital Policy Hub", "ui": "Poly-grid, Ray-traced refraction, Semi-transparent glass", "palette": "Steel Blue, Cyan, Silver", "highlight": "Neon Lime Green"},
    "綠能永續 (Eco-Future)": {"theme": "Sustainability & ESG", "ui": "Natural textures, Outdoor bokeh, Recycled paper", "palette": "Emerald Green, White", "highlight": "Sunlight Gold"},
    "現代民俗 (Modern Festive)": {"theme": "Modern Folk, Rich Vermilion", "ui": "Lacquered Wood, Silk texture, Traditional patterns", "palette": "Vermilion Red, Gold", "highlight": "Imperial Gold"},
    "生醫科技 (Clinical White)": {"theme": "Bio-Tech", "ui": "Clinical surfaces, DNA helix, Hexagonal grid", "palette": "Pristine White, Sky Blue", "highlight": "Bright Sky Blue"}
}
...
def build_final_prompt(title, left_in, right_in, style_name, header_mode, icon_style, ai_autonomy, ticker_lock):
    ...
    TICKER_ZONE = "[ABSOLUTE PHYSICAL VOID] Bottom-Right ($1332 < X < 1920$, $990 < Y < 1080$). NO text/effects." if ticker_lock else "[FULL CANVAS]"
...
- (圓括號): KEEP as is. NO color change.
- [圖----slug圖]: ASSET PROTECTION ZONE. Leave area 100% BLANK for photo insertion. DELETE label.
- [效果說明]: (e.g. [對話框], [圓餅圖], [日曆], [icon]). Render visual ONLY, DELETE label text.
...
"""
    ...
    return f"""
[SYSTEM V12.0] CANVAS: 1920x1080. {TICKER_ZONE}
{HEADER_W}
{SYMBOL_PROTOCOL}
[IMPORTANT] If "[圖----slug圖]" is present, leave area VOID. DO NOT overlap with DATA_A or DATA_B.
...
<body>
    ...
    <div class="toolbar">
        ...
        <div class="group"><button id="aiProtectBtn">🔒 AI 鎖定</button><button class="mode-btn" data-mode="refine_add" style="color:var(--cyan)">✨ 補回人像</button><button class="mode-btn" data-mode="refine_sub" style="color:var(--orange)">🔪 裁切邊緣</button></div>
    ...
    <script>
        ...
        workCanvas.onmousedown = e => {
            ...
            if(['brush','eraser'].includes(currentMode)){ 
                mCtx.beginPath(); mCtx.moveTo(p.x, p.y); mCtx.lineWidth = document.getElementById('brushSize').value; mCtx.lineCap = 'round';
                mCtx.globalCompositeOperation = currentMode==='eraser' ? 'destination-out' : 'source-over'; mCtx.strokeStyle = 'white'; mCtx.lineTo(p.x, p.y); mCtx.stroke(); 
            }
            render();
        };
        ...
        workCanvas.onmousemove = e => {
            ...
            else if (isDrawing && ['brush','eraser'].includes(currentMode)) { mCtx.lineTo(p.x, p.y); mCtx.stroke(); }
            render();
        };

        window.onmouseup = () => { 
            if (isDrawing && (currentMode==='rect'||currentMode==='circle')) { 
                activeShape = { type: currentMode, x: startX, y: startY, w: mouseX-startX, h: mouseY-startY }; 
            } 
            isDrawing = isMovingItem = isResizingItem = false; render(); 
        };

        function saveHistory() { 
            history.push({ manual: mCtx.getImageData(0,0,1920,1080), ai: aiCtx.getImageData(0,0,1920,1080) }); 
        ...
        function undo() { 
            if(history.length > 0) { 
                let h = history.pop(); 
                mCtx.putImageData(h.manual, 0, 0); 
                aiCtx.putImageData(h.ai, 0, 0); 
                render(); 
            } 
        }

        function commitShape() { 
            if (!activeShape) return; saveHistory(); 
        ...
        window.onkeydown = e => { if(e.key.toLowerCase()==='z' && (e.ctrlKey || e.metaKey)) { e.preventDefault(); undo(); } };
    </script>
</body>
</html>
"""
...
# 4. Streamlit 介面
# ==========================================
st.set_page_config(page_title="Visual Director v12.0", layout="wide")
st.title("🎬 Visual Director v12.0 - 終極修復完全體")
st.caption("Producer Huifen Edition | 18年導播魂品質保證")

tab1, tab2 = st.tabs(["🚀 第一步：鏡面指令", "🖍️ 第二步：華視打洞機"])

with tab1:
    with st.expander("❓ 鏡面符號矩陣與轉換原則 (點擊展開)", expanded=True):
        st.markdown("### 🔠 符號語法說明")
        st.table([
            {"語法": '"文字"', "視覺動作": "文字變色 (Highlight)", "處理": "❌ 移除引號"},
            {"語法": "【文字】", "視覺動作": "色塊小標 (字大大大)", "處理": "❌ 移除括號"},
            {"語法": "(圓括號)", "視覺動作": "輔助資訊感", "處理": "✅ 保留圓括號"},
            {"語法": "[圖----slug圖]", "視覺動作": "🛡️ 留白避讓特區", "處理": "❌ 移除標籤"},
            {"語法": "[效果說明]", "視覺動作": "轉化為圖示 (如：[對話框])", "處理": "❌ 移除說明"},
            {"語法": "[換行]", "視覺動作": "強制換行指令", "處理": "❌ 移除標籤"}
        ])
    ...
    with col_l:
        st.subheader("📋 內容編輯")
        h_mode = st.radio("標題張力：", ["兩行大標題", "單行大標題"], horizontal=True)
        title_in = st.text_area("主標題 (AI字體大大大)", value='"芬太尼喪屍"入侵萬華?[換行]\n"對折人"驚悚影像曝光', height=100)
        c1, c2 = st.columns(2)
        with c1: left_in = st.text_area("區塊 A", value='[threads icon] \n[網友icon] 網友:[對話框] \n我以為我在舊金山\n[圖----網友圖佔版面1/2]\n北市萬華1男子\n遭疑吸"芬太尼"', height=200)
        with c2: right_in = st.text_area("區塊 B", value='【"芬太尼"小檔案】\n●第二級毒品\n●美加地區濫用\n  造成死亡案例\n\n刑事局:\n觀看影像確有吸毒樣子', height=200)
    
    with col_r:
        st.subheader("🛠️ 視覺與空間規格")
        ticker_lock = st.toggle("🔒 鎖定 588x90 跑馬燈禁區", value=True)
        s_style = st.selectbox("旗艦風格庫", list(STYLE_CONFIG.keys()), index=3) 
        ai_sovereignty = st.toggle("✨ AI 視覺主權", value=True)
        r1, r2 = st.columns(2)
        with r1: s_layout = st.radio("排版", ["GRID", "DYNAMIC"], horizontal=True)
        with r2: s_icon = st.radio("物件", ["Flat", "Volumetric"], index=1, horizontal=True)

    if title_in:
        st.divider()
        st.subheader("🔥 Gemini 繪圖指令")
